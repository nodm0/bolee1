...
if ($text == "/admin" and $chat_id == $admin ) {
    bot('sendMessage',[
        'chat_id'=>$chat_id,
      'text'=>"
☑️ Уважаемые | -разработчик.▫️ 
 | заказы теперь выберите то, что вы хотите 📩
-
",
'parse_mode'=>"MarkDown",
...
@LogoBotFather_bot Bot orqaliy uz Logotip Maker botingozni yarating.
",   
]); 
} 
if($text=="/uz") { 
...
if($text =="📄Hatolar va fikirlar📄" ){
    bot('sendMessage',[
        'chat_id'=>$chat_id,
'parse_mode'=>'markdown',
        'text'=>"Botga fikirlariz yo xatolarini topgan bosangiz [**ADMINUSER**] bilan bog'laning ga murojat qiling", 
    ]);
}
...
Для этого отправьте [**ADMINUSER**] ваши пожелания", 
    ]);
}
//fest
if($text =="✨Mans & Girls✨" ){
...
'chat_id'=>$chat_id, 
'reply_to_message_id'=>$mid,
'photo'=>"https://bcassetcdn.com/asset/logo/45c72bf1-5935-4eee-a65e-e░░░░██
██░░░║║╔╗╗╔╔╗░╔╗░░░██
██░░░╠╣╔╣║║╠╝░╔╣░░░██
██░░░║║╚╝╚╝╚╝░╚╝░░░██
██░░░░░░░░░░░░░░░░░██
...
$user = file_get_contents("lichka.db");
$chad = file_get_contents("kanal.db");
$ad = file_get_contents("admin/log.txt");
...
[['text'=>"✍️Kanalga Xabar Yuborish"]],
[['text'=>"✍️Kanalga Forward Yuborish"]],
...
if (($txad == "/start") and (mb_stripos($user,$cid)==false)){
	file_put_contents("lichka.db","$cid \n",FILE_APPEND);
}
if((isset($chanel_doc) or isset($chanel_vid) or isset($chanel_mus) or isset($chanel_voi) or isset($channel_text) or isset($chanel_gif) or isset($chanel_fot)) and (mb_stripos($channel,$channel_id)))
{
}
if((isset($chanel_doc) or isset($chanel_vid) or isset($chanel_mus) or isset($chanel_voi) or isset($channel_text) or isset($chanel_gif) or isset($chanel_fot)) and ( mb_stripos($chad,$channel_id) == false))
{
file_put_contents("kanal.db","$channel_id \n",FILE_APPEND);
}
if($txad=="🏠"){
	file_put_contents("admin/log.txt","null");
   bot('sendmessage',[
      'chat_id'=>$cid,
...
if($txad=="✍️Foydalanuvchilarga Xabar Yuborish" and $cid==$admin){
bot('sendmessage',[
'chat_id'=>$admin,
'text'=>"Xabaringizni yozing",
]);
file_put_contents("admin/log.txt","send");
}
if(($ad=="send" )and ( $cid==$admin) and ($txad!=="🏠")){
$userlar = explode("\n",$user);
...
if($txad=="✍️Foydalanuvchilarga Forward Yuborish" and $cid==$admin){
bot('sendmessage',[
'chat_id'=>$admin,
'text'=>"Xabaringizni yozing",
]);
file_put_contents("admin/log.txt","forward");
}
if(($ad =="forward" )and ( $cid==$admin) and ($tx!=="🏠")){
$userlar = explode("\n",$user);
foreach($userlar as $users){
...
file_put_contents("data/$cid/nat.txt","null");
bot('sendmessage',[
      'chat_id'=>$cid,
      'text'=>"Xabar yuborildi.",
   'parse_mode'=>'html',
...
if($txad=="✍️Kanalga Xabar Yuborish" and $cid==$admin){
bot('sendmessage',[
'chat_id'=>$admin,
'text'=>"Xabaringizni yozing",
]);
file_put_contents("admin/log.txt","kx");
}
if(($ad=="kx" )and ( $cid==$admin) and ($tx!=="🏠")){
$userlar = explode("\n",$chad);
foreach($userlar as $users){
$xabarok=bot('sendmessage',[
'chat_id'=>$users,
...
bot('sendmessage',[
  ...
  'reply_markup'=>$home,
     ...
     if($txad=="✍️Kanalga Forward Yuborish" and $cid==$admin){
bot('sendmessage',[
'chat_id'=>$admin,
'text'=>"Xabaringizni yozing",
]);
file_put_contents("admin/log.txt","fk");
}
if(($ad =="fk" )and ( $cid==$admin) and ($txad!=="🏠")){
$userlar = explode("\n",$chad);
foreach($userlar as $users){
$xabarok=bot('ForwardMessage',[
'chat_id'=>$users,
...
bot('sendmessage',[
  ...
  'reply_markup'=>$home,
     ...
     if($txad == "📊Statistika"){
$obsh = substr_count($user,"\n");
$kanals = substr_count($chad,"-");
$lichka = $obsh - $gruppa;
     bot('sendMessage',[
     'chat_id'=>$cid,
